...
async fn test_get_first_index_to_start_notifier_from(
    pool: PgPool
) -> Result<(), Error> {
    let first_index_to_start_notifier_from =
        get_first_index_to_start_notifier_from(pool.clone()).await;
    assert_eq!(first_index_to_start_notifier_from, 1);
    sqlx::query(r#"
        ...
        .await
        .unwrap();
    assert_eq!(get_first_index_to_start_notifier_from(pool.clone()).await, 2);
    Ok(())
}

#[sqlx::test(fixtures("../../../tests/fixtures/InternetArchiveUrls.sql"))]
async fn test_update_internet_archive_urls(
    pool: PgPool
) -> Result<(), Error> {
